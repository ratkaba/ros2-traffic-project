...
class Patrol_Feedback(metaclass=Metaclass_Patrol_Feedback):
    ...
    def __init__(self, **kwargs):
        ...
        if self._check_fields:
            assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
                'Invalid arguments passed to constructor: %s' % \
                ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        self.left_time = kwargs.get('left_time', float())
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if self.left_time != other.left_time:
            return False
        return True
...
class Metaclass_Patrol_SendGoal(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'turtlebot3_msgs.action.Patrol_SendGoal')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
...
class Metaclass_Patrol_GetResult(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'turtlebot3_msgs.action.Patrol_GetResult')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
...
class Metaclass_Patrol_FeedbackMessage(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'turtlebot3_msgs.action.Patrol_FeedbackMessage')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
...
class Patrol(metaclass=Metaclass_Patrol):
    ...
    from turtlebot3_msgs.action._patrol import Patrol_Goal as Goal
    # The result message defined in the action definition.
    from turtlebot3_msgs.action._patrol import Patrol_Result as Result
    # The feedback message defined in the action definition.
    from turtlebot3_msgs.action._patrol import Patrol_Feedback as Feedback

    class Impl:

        # The send_goal service using a wrapped version of the goal message as a request.
        from turtlebot3_msgs.action._patrol import Patrol_SendGoal as SendGoalService
        # The get_result service using a wrapped version of the result message as a response.
        from turtlebot3_msgs.action._patrol import Patrol_GetResult as GetResultService
        # The feedback message with generic fields which wraps the feedback message.
        from turtlebot3_msgs.action._patrol import Patrol_FeedbackMessage as FeedbackMessage
